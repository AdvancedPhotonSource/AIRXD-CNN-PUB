...
class Dataset:
    ...
    def Make2ThetaAzimuthMap(self, data, iLim, jLim):
        '''Creates a corrected 2 theta azimuth map factoring in:
        X/Y pixel size
        ...
        #Meshgrid with x limits of (0.5, x_size + 0.5) and y limits of (0.5, y_size + 0.5)
        tay,tax = np.mgrid[iLim[0]+0.5:iLim[1]+.5,jLim[0]+.5:jLim[1]+.5]
        
        #Conver to float, scaling up the dimensions of each pixel
        ...
        #TA contains all corrected maps
        TA = np.empty((4,nI,nJ))
        #Call getTthAzmG2 to get corrected maps
